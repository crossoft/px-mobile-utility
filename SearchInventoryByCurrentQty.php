...
{
    global $conn;
    //$q['query'] ="SELECT name,minLevel,maxLevel,calculatedQty FROM `inventory_products` WHERE accountId='$accountId' AND name like '%$Name%'  ORDER BY name";
     $q['query'] ="SELECT * FROM `inventory_products` WHERE accountId='$accountId' AND name like '%$Name%'  ORDER BY name";
