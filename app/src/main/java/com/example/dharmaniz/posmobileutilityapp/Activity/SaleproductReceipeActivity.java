...
import com.example.dharmaniz.posmobileutilityapp.Adapter.SaleProductRecipeAdapter;
...
public class SaleproductReceipeActivity  extends PostMobileBaseActivity {
    Activity mActivity = SaleproductReceipeActivity.this;
    ...
    SaleProductRecipeAdapter mAdapter;
    ArrayList<SaleProductReciepeModel> mArrayList = new ArrayList<SaleProductReciepeModel>();

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_saleproduct_receipe);
    }
    ...
    private void ExcecuteSerachAPI(String strSearchText) {
        ...
        String url = "http://dharmani.com/jspro2/SearchSalesInventoryProductsByRecipe.php";
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("salesProductName", strSearchText);
...
////      Adding request to request queue
        ...
        try {
            ...
            if (!responce.getString("status").equals("")) {
                ...
                for (int i = 0; i < mJsonArray.length(); i++) {
                    ...
                    SaleProductReciepeModel mSaleModel = new SaleProductReciepeModel();
                    if (!jsonDATAObject.getString("salesProductName").equals("")) {
                        mSaleModel.setInventoryProductName(jsonDATAObject.getString("salesProductName"));
                    }
                    if (!jsonDATAObject.getString("qty").equals("")) {
                        mSaleModel.setQty(jsonDATAObject.getString("qty"));
                    }
                    if (!jsonDATAObject.getString("unitName").equals("")) {
                        mSaleModel.setUnitName(jsonDATAObject.getString("unitName"));
                    }
                    if (!jsonDATAObject.getString("instructions").equals("")) {
                        mSaleModel.setInstructions(jsonDATAObject.getString("instructions"));
                    } if (!jsonDATAObject.getString("isCommon").equals("")) {
                        mSaleModel.setInstructions(jsonDATAObject.getString("isCommon"));
                    }
                    if (!jsonDATAObject.getString("product_id").equals("")) {
                        mSaleModel.setProduct_id(jsonDATAObject.getString("product_id"));
                    }
    ...
    private void setAdapter() {
        ...
        mAdapter = new SaleProductRecipeAdapter(mActivity, mArrayList);
